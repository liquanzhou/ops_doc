...
因为监视只是监视之后发生的改变，所有需要先get在watch，这里有一个index的值

./etcdctl  ls /foo
./etcdctl  get /foo/new_bar

# 一直监测一个值
./etcdctl watch /foo/new_bar --forever |while read a;do echo $a;done
# 改值
./etcdctl update /foo/new_bar "hello etc412q"

# 监视值改变执行操作
./etcdctl exec-watch testkey -- sh -c 'ls'

# 监视目录下所有改变
./etcdctl watch /foo/ --forever --recursive
...
GLOBAL OPTIONS:
   ...
   --no-sync            dont synchronize cluster information before sending request
